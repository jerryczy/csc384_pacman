...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        for m in range(i, 8):
            for n in range(j, 8):
                if m != n and abs(m-n) != abs(i-j):
                    cons_set.append([m, n])
        TableConstraint.__init__(self, name, [qi, qj], cons_set)
...
class NValuesConstraint(Constraint):
    ...
    def hasSupport(self, var, val):
        ...
        def nValue_partial(l):
            vals = [val for (var, val) in l]
            count = vals.count(self._required)
            remainsNum = len(self.scope()) - len(vals)
            return (count + remainsNum) >= self._lb and count <= self._ub

        varsToAssign = self.scope()
        varsToAssign.remove(var)
        return findvals(varsToAssign, [(var, val)], nValue_final, nValue_partial)

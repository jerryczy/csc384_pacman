...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        isGoal = position in self.corners and not unvisited
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            nextx, nexty = int(x + dx), int(y + dy)
            unv = list(unvisited)
            if (nextx, nexty) in unv:
                unv.remove((nextx, nexty))
            if not self.walls[nextx][nexty]:
                nextState = ((nextx, nexty), tuple(unv))
...
def cornersHeuristic(state, problem):
    ...
    for corner in unvisited:
        total += abs(position[0] - corner[0]) + abs(position[1] - corner[1])
    import math
    if total > 0:
        total = math.log(total) ** 2.38   # 2.38 ~ 2.49
    return total
...
def foodHeuristic(state, problem):
    ...
    for point in foodGrid.asList():
        total += mazeDistance(position, point, problem.startingGameState)
    return total ** 0.615 # 7125
